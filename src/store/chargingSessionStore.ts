...
interface ChargingSessionStore extends ChargingSessionUiState {
  ...
  loadChargingSessionsByTravelEvent: (travelEventId: string, page?: number) => Promise<void>;
...
export const useChargingSessionStore = create<ChargingSessionStore>((set) => ({
  ...
  loadChargingSessionsByTravelEvent: async (travelEventId: string, page = 0) => {
    set({ loading: true, error: null });
    try {
      const sessions = await PerformanceMonitor.measureAsync(
        'loadChargingSessionsByTravelEvent',
        () => repository.getByTravelEventIdPaginated(travelEventId, page, PAGINATION.itemsPerPage)
      );
      set({ chargingSessions: sessions, currentPage: page, loading: false });
    } catch (error) {
      set({ error: (error as Error).message, loading: false });
